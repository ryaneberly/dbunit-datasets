 ...
 */
public class AttributeSymbolReplacementFlatXmlProducer extends FlatXmlProducer implements IDataSetProducer, ContentHandler
{

    /**
     * Logger for this class
     */
    private static final Logger logger = LoggerFactory.getLogger(AttributeSymbolReplacementFlatXmlProducer.class);
    ...
    public AttributeSymbolReplacementFlatXmlProducer(InputSource xmlSource)
    {
        this(xmlSource, true);
    }

    public AttributeSymbolReplacementFlatXmlProducer(InputSource xmlSource, boolean dtdMetadata)
    {
        this(xmlSource, dtdMetadata, false);
    }

    public AttributeSymbolReplacementFlatXmlProducer(InputSource xmlSource, IDataSet metaDataSet)
    {
    ...
    public AttributeSymbolReplacementFlatXmlProducer(InputSource xmlSource, EntityResolver resolver)
    {
     ...
     */
    public AttributeSymbolReplacementFlatXmlProducer(InputSource xmlSource, boolean dtdMetadata, boolean columnSensing)
    {
        this(xmlSource, null, dtdMetadata, columnSensing, false);
    }
     ...
     * @since 2.4.2
     */
    public AttributeSymbolReplacementFlatXmlProducer(InputSource xmlSource, Map<String,String> replacements,boolean dtdMetadata, boolean columnSensing, boolean caseSensitiveTableNames)
    {
    ...
    private ITableMetaData createTableMetaData(String tableName, Attributes attributes) throws DataSetException
    {
        ...
        {
            columns[i] = new Column(transformColumnName(attributes.getQName(i)), DataType.UNKNOWN);
        }
	...
	private final String transformColumnName(final String colName){
		String columnName = colName; //mutable
		for(final Entry<String, String> entry: _replacements.entrySet()){
			while(columnName.contains(entry.getKey())){
			...
			throws DataSetException
	{
		...
		{
			try {
			    activeMetaData.getColumnIndex(transformColumnName(attributes.getQName(i)));
			} 
			catch (NoSuchColumnException e) {
				columnsToMerge.add(new Column(transformColumnName(attributes.getQName(i)), DataType.UNKNOWN));
			}
		}
            ...
            if (attributes.getLength() > 0)
            {
                ...
                for (int i = 0; i < columns.length; i++)
                {
                	...
                	for(int j=0;j<attributes.getLength(); j++){
                		if(column.getColumnName().equals(transformColumnName(attributes.getQName(j)))){
                			rowValues[i] = attributes.getValue(j);
                		}
                	}
                }
    ...
    private static class FlatDtdHandler extends FlatDtdProducer
    {
        ...
        private AttributeSymbolReplacementFlatXmlProducer xmlProducer;

        public FlatDtdHandler(AttributeSymbolReplacementFlatXmlProducer xmlProducer)
        {
